...
#[test]
fn michaelis_menten_consumption_lowers_oxygen_and_produces_carbon_dioxide() {
    ...
    assert!(oxygen_output.iter().all(|value| *value < 1.0));
    assert!(carbon_dioxide_output.iter().all(|value| *value > 0.0));
}
