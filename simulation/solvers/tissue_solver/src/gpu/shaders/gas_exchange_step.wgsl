...
// reset_vessels -> Whether vessel O₂ resets and vessel CO₂ sink/source terms apply.
...
//
// Vessel cells optionally reset O₂ to a fixed concentration and apply CO₂
// as a source/sink term.
// -----------------------------------------------------------------------------
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
	...
	let updated_gas = center + params.dt * (gas_diffusion + reaction);

	if (params.reset_vessels == 1u && vessel_mask[idx] != 0u) {
		let vessel_values = vessel_concentration[idx];
		gas_next[idx] = vec2<f32>(
			vessel_values.x,
			max(0.0, updated_gas.y + params.dt * vessel_values.y),
		);
		return;
	}

	gas_next[idx] = updated_gas;
}