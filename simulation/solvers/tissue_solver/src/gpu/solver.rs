...
impl WgpuGasExchangeSolver {
	...
	/// O₂/CO₂ concentrations, O₂/CO₂ diffusivities, `vmax`/`km`, vessel O₂
	/// concentration targets, and vessel CO₂ source/sink terms. The shader then
	/// advances both species together so oxygen consumption and carbon dioxide production stay numerically coupled.
	///
	...
	///
	/// Notes:
	/// Vessel O₂ is reset every timestep on the GPU path (`reset_vessels = true`)
	/// to model persistent oxygen delivery. Vessel CO₂ is interpreted as a
	/// source/sink term, so negative values remove CO₂ instead of pinning vessel
	/// CO₂ to a fixed concentration.
	pub async fn run_gas_exchange_steps(
		...
		// Pack vessel terms as [O₂ concentration target, CO₂ source/sink term].
