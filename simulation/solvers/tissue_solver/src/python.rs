...
/// vessel_o2              -> Fixed oxygen concentration target imposed on vessel cells.
/// vessel_co2             -> Carbon dioxide source/sink term applied at vessel cells.
...
/// reset_vessels          -> Whether vessel O₂ resets and vessel CO₂ terms apply after each update.
///
/// Returns:
...
pub fn run_gas_exchange_steps_auto_numpy<'py>(
    ...
    let input = GasExchangeStepInput {
        ...
        // `vessel_co2` is still routed through the existing field name, but the
        // solver interprets it as a CO₂ source/sink term rather than a fixed
