...
/// produced from the oxygen consumption rate, and is removed or added at vessel cells
/// through the vessel CO₂ source/sink term.
///
/// Parameters:
...
/// vessel_carbon_dioxide_concentration  -> Vessel CO₂ source/sink term applied after diffusion/production.
/// dt                                   -> Timestep size.
    ...
    if input.reset_vessels {
        for idx in 0..grid.len() {
            if input.vessel_mask[idx] {
                ...
                input.carbon_dioxide_next[idx] = (input.carbon_dioxide_next[idx]
                    + dt * input.vessel_carbon_dioxide_concentration[idx])
                    .max(0.0);
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn gas_exchange_step_applies_vessel_carbon_dioxide_sink_term() {
        let grid = Grid2D::new(3, 3, 1.0, 1.0);
        let oxygen = vec![5.0; grid.len()];
        let carbon_dioxide = vec![1.0; grid.len()];
        let diffusivity = vec![0.0; grid.len()];
        ...
        vessel_mask[center_idx] = true;
        let vessel_oxygen = vec![10.0; grid.len()];
        let vessel_carbon_dioxide = vec![-2.0; grid.len()];
        let mut oxygen_next = vec![0.0; grid.len()];
        let mut carbon_dioxide_next = vec![0.0; grid.len()];
        ...
        assert_eq!(oxygen_next[center_idx], 10.0);
        assert!((carbon_dioxide_next[center_idx] - 0.5).abs() < 1e-6);
    }
}
